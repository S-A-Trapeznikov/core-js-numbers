...
function toNumber(value, def) {
  ...
  return Number.isNaN(castedValue) ? def : castedValue;
}
...
function getFibonacciNumber(index) {
  let beforeLast = 0;
  let lastValue = 1;
  for (let k = 2; k <= index; k += 1) {
    const temp = beforeLast;
    beforeLast = lastValue;
    lastValue += temp;
  }
  return index === 0 ? beforeLast : lastValue;
}
...
function getCountOfOddNumbers(number) {
  const cNum = Math.abs(number);
  return cNum % 2 === 0 ? cNum / 2 : (cNum + 1) / 2;
}
